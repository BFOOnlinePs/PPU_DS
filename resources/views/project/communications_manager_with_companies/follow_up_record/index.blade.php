...
@section('header_link')
    <a
        href="{{ route('communications_manager_with_companies.students.index') }}">{{ __('translate.Students') }}{{-- الطلاب --}}</a>
@endsection
@section('style')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col">
                                            <div class="form-group">
                                                ...
                                                <select onchange="company_table_ajax()" class="form-control" name=""
                                                    id="company_status">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            company_table_ajax();
            search_student_ajax();
            $('#company_search').on('input', debounce(company_table_ajax, 800));
        });
                ...
                success: function(data) {
                    if (data.success == 'true') {
                        if (data.status === 'found') {
                            $('#validation_email').text(data.message).removeClass('text-success').addClass(
                                'text-danger');
                            $('#contact_company_save_button').prop('disabled', true);
                        } else {
                            $('#validation_email').text(data.message).removeClass('text-danger').addClass(
                                'text-success');
