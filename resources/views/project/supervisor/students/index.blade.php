...
@endsection
@section('header_link')
    <a href="{{ route('admin.users.details', ['id' => $user->u_id]) }}">{{ $user->name }}</a> /
    {{ __("translate.Academic Supervisor's Students") }}{{-- طلاب المشرف --}}
@endsection
            ...
            <div class="row">
                ...
                <form class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-8">
                                <input class="form-control mb-2 " onkeyup="user_search(this.value)" type="search"
                                    placeholder="{{ __('translate.Search') }}"> {{-- بحث --}}
                            </div>
                            <div class="col-md-4">
                                <select autofocus class="js-example-basic-single col-sm-12" name="m_id" id="select-major"
                                    onchange="select_major(this.value)">
                                    @if (isset($major))
                                        <option value="{{ $major->m_id }}">{{ $major->m_name }}</option>
                                    @endif
                                    <option value="{{ null }}">
                                        {{ __('translate.All Majors') }}{{-- جميع التخصصات --}}</option>
