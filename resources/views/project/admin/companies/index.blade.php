...
@section('content')
    ...
    <div>
        <button class="btn btn-primary  mb-2 btn-s" type="button"
            onclick='location.href="{{ route('admin.companies.company') }}"'><span class="fa fa-plus"></span>
            {{ __('translate.Add Company') }}{{-- إضافة شركة --}}</button>
        <button class="btn btn-primary  mb-2 btn-s" type="button"
            onclick='location.href="{{ route('admin.companies_categories.index') }}"'><span class="fa fa-briefcase"></span>
            {{ __('translate.Companies Categories') }}{{-- تصنيف الشركات --}}</button>
    </div>
